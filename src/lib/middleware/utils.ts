...
import { isHomeHostname } from '../web';

export const parse = (req: NextRequest) => {
  let domain = req.headers.get('host') as string;
  domain = domain.replace('www.', ''); // remove www. from domain
  if (isHomeHostname(domain)) domain = 'mindplanr.com'; // if domain is a home hostname, set it to mindplanr.com
  const path = req.nextUrl.pathname;
  const key = decodeURIComponent(path.split('/')[1]); // decodeURIComponent to handle foreign languages like Hebrew
